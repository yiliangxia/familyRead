	...
	<ul class="fancy pagination">
		<c:choose>
			<c:when test="${page.pageNo > 1}">
				<li><a
					href="javascript:loadBannerList(${page.prePage},${page.pageSize})"><<</a></li>
			</c:when>
			<c:otherwise>
		...
		<c:forEach var="i" begin="${begin}" end="${end}">
			<c:choose>
				<c:when test="${i == current}">
					<li><a class="active"
						href="javascript:loadBannerList(${i},${page.pageSize})">${i}</a></li>
				</c:when>
				<c:otherwise>
					<li><a
						href="javascript:loadBannerList(${i},${page.pageSize})">${i}</a></li>
		...
		<c:choose>
			<c:when test="${page.hasNext}">
				<li><a
					href="javascript:loadBannerList(${page.nextPage},${page.pageSize})">>></a></li>
			</c:when>
			<c:otherwise>
				<li><a href="javascript:void(0)">>></a></li>
			</c:otherwise>
